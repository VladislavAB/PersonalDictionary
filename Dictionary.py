...
#  Берет слово из API и возвращает строки: определение, примеры и часть речи.
...
def create_dict_file(dict_path: str, first_data: str)-> True:
    ...
    with open(dict_path, 'w') as f:
        ...
        f.write(row)
    return True
# Создает в первый раз словарь csv и записывает первое word info.
def append_dict(dict_path: str, word_info: str)-> True:
    ...
    with open(dict_path, 'a') as f:
        f.write(row)
    return True
# Добавляет последующие word info.
...
def read_stats(dict_stat_path: str)-> dict:
    ...
    return stats_dict
# Читаем статистику в dict, если она есть. Если ее нет, то просто возвращаем пустой словарь.
def update_dict_stats(dict_stat_path: str, word: str)-> dict:
    ...
    return stats
# Если из read пришел пустой словарь или слова нет в словаре, то записыаем его. Если есть слово в словаре, +1.
